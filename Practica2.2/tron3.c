...
/*     tercera versio del joc, nomes hi ha un tron que controla l'usuari, i    */
...
/*	   $ ./tron3 num_oponents variabilitat fitxer [retard] 		       */
/*									       */
/*  Codis de retorn:						  	       */
...
{
  int n, i, mida, id_camp, id_fi1, id_oponents_morts, *p_camp, index;		/* variables locals */
  ...
  if (n_args <= 4)
  {	fprintf(stderr,"Comanda: ./tron3 num_oponents variabilitat fitxer [retard]\n");
  ...
  *oponents_morts = 0; 
  ...
  for (i = 0; i < n_oponents; i++) 
  {
	...
	if (tpid_opo[n] == (pid_t) 0) 
	{
		...
		execlp("./oponent3", "oponent3", a1, a2, a3, a4, a5, a6, a7, a8, a9, ll_args[3], (char *) 0);
		fprintf(stderr,"Error: No es pot executar el procés fill \'oponent3\'\n");
		fclose(fitxer);
  		elim_mem(id_camp);
  		elim_mem(id_fi1);
