...
/*	   $ ./tron1 num_oponents variabilitat fitxer [retard] 		     */
/*									     */
/*  Codis de retorn:						  	     */
...
void * mou_oponent(void * index)
{
  ...
  do 
  {
	canvi = 0;
  	...
  	else esborrar_posicions('1' + (intptr_t) index, p_opo, n_opo);
	win_retard(retard);
  } while (!fi1 && !fi2);
...
{
  int n_oponents, n, i, retwin, minuts, segons;		/* variables locals */
  char strin[45];
  ...
  if (n_args <= 4)
  {	fprintf(stderr,"Comanda: ./tron1 num_oponents variabilitat fitxer [retard]\n");
